...
if [ $? -eq 0 ]; then
    ...
    a developer install of jupyterlab_react you must have pip installed on
...
if [ $? -eq 0 ]; then
    ...
    if [[ "$OSTYPE" == "msys" ]]; then
        jupyter labextension install --py $nbExtFlags jupyterlab_react
    else
        jupyter labextension install --py --symlink $nbExtFlags jupyterlab_react
    fi
    jupyter labextension enable --py $nbExtFlags jupyterlab_react
else
    echo "'jupyter lab --version' failed, therefore jupyter lab is not installed. In
    order to perform a developer install of jupyterlab_react you must
...
if [ $? -eq 0 ]; then
    ...
    if [[ "$OSTYPE" == "msys" ]]; then
        jupyter nbextension install --py $nbExtFlags jupyterlab_react
    else
        jupyter nbextension install --py --symlink $nbExtFlags jupyterlab_react
    fi
    jupyter nbextension enable --py $nbExtFlags jupyterlab_react
else
    ...
    jupyterlab_react you must have jupyter notebook on your machine!
